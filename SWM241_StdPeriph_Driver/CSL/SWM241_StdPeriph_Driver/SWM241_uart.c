...
{	
	...
	{
	case ((uint32_t)UART0):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn) NVIC_EnableIRQ(UART0_IRQn);
		break;
	
	case ((uint32_t)UART1):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn) NVIC_EnableIRQ(UART1_IRQn);
		break;
	
	case ((uint32_t)UART2):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn) NVIC_EnableIRQ(UART2_IRQn);
		break;
	
	case ((uint32_t)UART3):		
		if(initStruct->RXThresholdIEn | initStruct->TXThresholdIEn | initStruct->TimeoutIEn) NVIC_EnableIRQ(UART3_IRQn);
		break;
	}
}
